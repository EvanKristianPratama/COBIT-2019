...
class AssessmentReportController extends Controller
{
    ...
    public function show($evalId)
    {
        try {
            ...
            return view('assessment.report.index', array_merge(
    ...
    public function index()
    {
        try {
            ...
            if (isset($data['error'])) {
                return view('assessment.report.all', [
                    'objectives' => [], 'assessments' => [],
                    'scopeMaturityData' => [], 'error' => $data['error'],
                ]);
            }

            return view('assessment.report.all', $data);

        } catch (\Exception $e) {
            Log::error('Failed to load all-years report', ['user_id' => Auth::id(), 'error' => $e->getMessage()]);

            return redirect()->route('assessment.index')->withErrors(['error' => 'Failed to load report: '.$e->getMessage()]);
        }
    }
    ...
    public function spiderweb()
    {
        try {
            $data = $this->assessmentReportService->buildOverviewReport(Auth::user());
            if (isset($data['error'])) {
                return view('assessment.report.spiderweb', [
                    'objectives' => [], 'assessments' => [],
                    'scopeMaturityData' => [], 'error' => $data['error'],
                ]);
            }

            return view('assessment.report.spiderweb', $data);
