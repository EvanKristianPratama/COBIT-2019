...
class AssessmentEvalController extends Controller
{
    ...
    public function createAssessment(CreateAssessmentRequest $request)
    {
        try {
            ...
            return redirect()->route('assessment.show', ['evalId' => $evaluation->encrypted_id]);
        } catch (\Exception $e) {
    ...
    public function showAssessment($evalId)
    {
        try {
            ...
            if (! $this->assessmentAccessService->canView($currentUser, $evaluation)) {
                return redirect()->route('assessment.index')->withErrors(['error' => 'Assessment not found or access denied']);
            }
            ...
            return view('assessment.assessmentDetail.index', $data);
        } catch (\Exception $e) {
            ...
            return redirect()->route('assessment.index')->withErrors(['error' => 'Failed to load assessment: '.$e->getMessage()]);
        }
    }
