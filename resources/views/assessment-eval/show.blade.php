        ...
        <div class="card-header hero-header py-4" style="background:linear-gradient(135deg,#081a3d,#0f2b5c);color:#fff;border:none;">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap justify-content-end">
                    ...
                    {{-- JSON Data for Scope --}}
                    <script>
                        window.ALL_OBJECTIVES_GROUPED = @json($allObjectives->groupBy(function($item) {
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header border-bottom-0 pb-0">
                    <div>
                        <h5 class="modal-title fw-bold text-primary">Edit Scope</h5>
                        <p class="text-muted small mb-0">Select the Governance & Management Objectives relevant to this assessment.</p>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
                ...
                <div class="card border-0 shadow-sm" style="border-radius: 12px;">
                    <div class="card-header bg-white border-bottom-0 pt-3 pb-2 d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center">
                            <span class="badge bg-primary bg-opacity-10 text-primary fs-6 me-2 px-3 py-2 rounded-pill">${domain}</span>
                            <span class="fw-bold text-dark">Domain ${domain}</span>
                        </div>
                        <div>
                            <button type="button" class="btn btn-xs text-primary fw-bold check-all" data-domain="${domain}" style="font-size:0.8rem">Select All</button>
                            <span class="mx-1 text-muted">|</span>
                            <button type="button" class="btn btn-xs text-muted uncheck-all" data-domain="${domain}" style="font-size:0.8rem">Clear</button>
                        </div>
                    </div>
                        ...
                        <div class="row g-2">
                            ${objectives.map(obj => `
                                <div class="col-6 col-md-4 col-lg-3">
                                    <label class="d-flex align-items-center p-2 rounded border bg-white h-100 position-relative scope-item-label" 
                                           style="cursor: pointer; transition: all 0.2s;">
                                        <input class="form-check-input me-3 mt-0 scope-checkbox" type="checkbox" 
                                               value="${obj.objective_id}" data-domain="${domain}" 
                                               style="width: 1.25em; height: 1.25em;">
                                        <div class="lh-sm">
                                            <div class="fw-bold text-dark small">${obj.objective_id}</div>
                                            <div class="text-secondary" style="font-size: 0.75rem;">${obj.objective}</div>
                                        </div>
                                    </label>
                                </div>
                            `).join('')}
        ...
        document.querySelectorAll('.check-all').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                document.querySelectorAll(`.scope-checkbox[data-domain="${domain}"]`).forEach(cb => {
                    cb.checked = true;
                });
                updateUI();
            });
        });
        document.querySelectorAll('.uncheck-all').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const domain = e.target.dataset.domain;
                document.querySelectorAll(`.scope-checkbox[data-domain="${domain}"]`).forEach(cb => {
                    cb.checked = false;
        ...
        saveBtn.addEventListener('click', function() {
            const selected = Array.from(document.querySelectorAll('.scope-checkbox:checked')).map(cb => cb.value);
