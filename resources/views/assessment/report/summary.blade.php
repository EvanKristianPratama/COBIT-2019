    ...
    <div class="container">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item">
                            <a href="{{ route('assessment.report', ['evalId' => $routeEvalId]) }}"
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route('assessment.report-activity', ['evalId' => $routeEvalId, 'objectiveId' => $objectives->first()->objective_id]) }}"
                                class="text-decoration-none text-muted" style="color: #0f2b5c !important;">
        ...
        @foreach ($objectives as $objective)
            <div class="mb-5">
                ...
                <div class="mt-2">
                    <form action="{{ route('assessment.summary.save-note', ['evalId' => $evaluation->eval_id]) }}"
                        method="POST">
                        @csrf
            ...
            <div class="sticky-action-group">
                <a href="{{ route('assessment.summary-pdf', ['evalId' => $routeEvalId, 'objectiveId' => $objective->objective_id]) }}"
                    class="btn btn-danger sticky-action-btn" target="_blank" title="Export PDF Per GAMO">
                    <i class="fas fa-file-pdf"></i>
                    <span>PDF Per GAMO</span>
                </a>
                <a href="{{ route('assessment.summary-detail-pdf', ['evalId' => $routeEvalId, 'objectiveId' => $objective->objective_id]) }}"
