...
const statusModalType = ref('pending');
const page = usePage();

// Get user info from flash
const statusUser = computed(() => page.props.flash?.status_user || null);

// Watch for flash messages
watch(() => page.props.flash?.status, (status) => {
    if (status === 'pending' || status === 'rejected' || status === 'deactivated') {
        statusModalType.value = status;
...
<template>
    ...
    <!-- Status Modal (Pending/Rejected/Deactivated) -->
    <StatusModal
